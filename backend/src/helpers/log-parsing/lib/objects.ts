...
export class Session {
  ...
  getDps() {
    ...
    const damage = this.entities.reduce((acc, e) => acc + e.stats.damageDealt, 0);
...
export class Skill {
  ...
  constructor(skill: {
    ...
    this.breakdown = skill.breakdown?.map((breakdown: SkillBreakdown) => new SkillBreakdown(breakdown)) || [];
...
export class SimpleEntity {
  ...
  constructor(entity: Entity) {
    ...
    this.skills = Object.values(entity.skills).map(skill => new SimpleSkill(skill));
